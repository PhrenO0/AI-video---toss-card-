# GPT Storyboard Continuity Bible

## Structural reference lock

- `references/structural/gate-hall-reference.jpg` and `right-concourse-reference.jpg` are architecture references only: do not copy or composite either photograph as the final background.
- A05 interprets the real gate hall's clearance, column depth, sign suspension and traffic spacing while retaining the project's modern stainless flap gates.
- A06 interprets the real corridor's ceiling rhythm, round-column row, service wall, tactile paving and physically mounted wayfinding.
- The `타는 곳 / Tracks` panel remains. It must be a slim matte-black physical panel with visible thickness, two narrow metal hanger rods, ceiling plates, correct perspective and a subtle underside shadow.
- Secondary signs stay distant or optically defocused. Never show crisp generated gibberish or repeat blue placards on every column.

## Spatial lock

- Portrait 9:16. Camera begins on the unpaid side, looking toward five realistic Korean subway flap gates.
- The center target lane is G3. G4 is the adjacent lane on screen-right.
- G3's contactless reader is on the tourist's right-hand gate housing, screen-right of her body.
- On failure, the tourist's shoes remain before the closed flap and the silver suitcase trails behind-left.
- The referee first appears 6–8m down the A06 right-side concourse, walks directly toward the tourist, then enters her near-gate frame right-to-left.
- Only after the referee taps the G3 reader does the G3 flap open and the tourist cross with the suitcase.
- A06 is the view obtained by turning the camera approximately 90° right from G3. It shares the same gray tile, restrained blue wayfinding, patterned floor and cool-white ceiling lights but contains no fare gates or `타는 곳` panel. S06 and S08 must share this exact corridor axis.
- The failure sequence is exactly three failed taps: S02 is the first failed tap in a strict rear wide view, S03A is the second failed tap from a 45-degree high angle with a small wrist correction, and S03B is the third failed tap from a reader-level 28–35mm bottom view with card and right hand in the foreground. S04 is face reaction only at 70–85mm, with gaze still down at the card-reader contact; it is not an attempt.
- Every failure-sequence prompt uses the same matte-gray unbranded card in the tourist's right hand and explicitly prohibits a TOSS card, camera eye contact and an extra attempt. S02 remains strict rear: no face or cheek is visible, and the suitcase trails diagonally behind-left.
- S05A is referee only: black futsal boots and black football socks in a floor-level start. S05B is referee only: the same referee remains on the same vertical body axis with whistle at mouth and stern face.
- S06 is a 35–50mm full-body referee shot in A06: head and boots must be visible, with no tourist silhouette.
- S13 has both black gate flaps fully retracted inside the housings, creating a clear walking and suitcase path. Keep the upper 30–35 percent quiet and copy-safe with all actors below 65 percent; use the right-hand card tag and keep the watch on the lowered left wrist.

## Character lock

- Tourist: the exact woman in `edit/generated/stills/tourist_lock_JP_v2.jpg`; preserve her full bangs, eye shape, jawline and shoulder-length dark-brown hair rather than generating a generic Japanese appearance. White loose blouse, light-blue straight jeans, beige flats, beige crossbody bag and silver hard-shell suitcase remain unchanged. Reactions stay small and realistic.
- Referee: the exact man in `edit/generated/stills/referee_lock_A.png`; late-30s/early-40s Korean man, athletic build, short black hair, black-and-white vertical striped referee shirt, black shorts, knee socks, black shoes, black whistle and black watch. Stern deadpan expression in every shot.
- Product: only `260702_토스_선불카드_애셋_모음집/pre-card-hologram-front (1).png`. It is absent S01–S09, partially visible S10, fully visible S11–S13.

## Visual lock

- Premium live-action Japanese television commercial, not travel UGC.
- Unmistakably Korean subway setting: gray tiled columns, functionally mounted route information and lived-in architectural depth. On the A05 gate axis, one slim black overhead panel reads `타는 곳 / Tracks`; on A06 the panel and gates are outside the right-facing field of view.
- Cool cyan-white Seoul subway fluorescent light, believable stainless steel, glass, rubber, fabric and skin texture, subtle film grain.
- Observational 28–35mm framing before the referee reveal; 50–70mm compression and low angles during the approach and card declaration.
- Japanese game-style blue-gold radial rays only for S10–S11. No full anime background replacement.
- No dialogue, speech balloons, subtitles, watermarks or fake brand marks. Preserve only the short primary panel copy; keep other background text too small or defocused to read.
- Every character frame captures action in progress: grounded feet, contact shadow, cloth or hair inertia and plausible suitcase-wheel direction. Avoid idle mannequin posing.

## Reject conditions

- Tourist beyond a closed flap, suitcase separated from her path, floating reader, merged lanes, duplicated gate housings.
- Face, hair, wardrobe, bag, suitcase, referee uniform, whistle or watch drift.
- Extra hands, fingers, arms, cards, people or mirrored anatomy.
- TOSS card before S10; inaccurate card artwork from S10 onward.
- S02 face or cheek visible; S02 tourist side-on to the gate; suitcase standing parallel without travel inertia.
- Any failure sequence other than exactly three failed taps (S02, S03A, S03B); a different card, TOSS card, camera eye contact or an extra attempt in S02/S03A/S03B/S04; S04 portrayed as an attempt, lacking its face-only 70–85mm downward-gaze reaction, or using the reader-level bottom viewpoint reserved for S03B.
- S05A containing the tourist, anything above the lower edge of the referee's shorts, or omitting the tight floor-level black futsal boots and black football socks; S05B containing the tourist or omitting the same referee's matched vertical body axis, whistle at mouth or stern face; S06 missing the referee's head or boots, or showing a tourist silhouette.
- S13 with gate flaps not fully retracted inside their housings, an obstructed walking/suitcase path, actors above 65 percent, a non-quiet upper 30–35 percent copy-safe area, a left-hand card tag or a raised/non-left wrist watch.
- S07 gaze away from screen-right; S09 referee moving left-to-right.
- Fare gates, gate readers or the `타는 곳` panel appearing in S06/S08; a tourist silhouette in S06; S08 without the same A06 approach direction.
- Floating, paper-thin or oversized signs; repeated blue placards; crisp unreadable generated text; perfect showroom symmetry.
