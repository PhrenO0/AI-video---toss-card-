...
**전환 링크:** 본문에 URL을 직접 넣지 않았다 — 인스타그램은 피드·릴스 캡션 안의 링크가 클릭되지 않는다.
실제 링크는 **프로필 편집 → 웹사이트** 칸에 등록한다.

```
https://toss.im/?pid=referral_partners_online_FORICREW09
```

X에 같은 영상을 올릴 경우에는 반대로 간다 — 본문에 링크가 있으면 도달이 눌리는 경향이 있어
**본문에는 링크 없이 발행하고, 발행 직후 첫 리플로 위 링크를 단다.**
(공식 링크 출처: [`content-workflow.md`](./content-workflow.md) §6-2)
